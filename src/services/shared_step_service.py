...
class SharedStepService:
    ...
    async def _upload_attachment(self, shared_step_id: int, att: dict[str, str]):
        ...
        try:
             ...
             raise AllureValidationError(f"Invalid base64 content: {e}")
             
        # client.upload_shared_step_attachment returns List[Row]
        rows = await self._client.upload_shared_step_attachment(
            shared_step_id,
            [(name, decoded)]
        )
        if not rows:
