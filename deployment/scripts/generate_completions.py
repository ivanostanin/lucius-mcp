...
for the entity/action CLI model.
"""
...
def _completion_data() -> tuple[list[str], dict[str, str], dict[str, list[str]]]:
    """
    Return:
    - all entity aliases accepted by CLI
    - alias -> canonical entity
    - canonical entity -> action aliases accepted by CLI
    """
...
def main() -> None:
    ...
    bash_completion = generate_bash_completion(entities, alias_to_canonical, actions_by_entity)
    (COMPLETION_DIR / "lucius.bash").write_text(bash_completion)
    print(f"Generated: {COMPLETION_DIR / 'lucius.bash'}")

    zsh_completion = generate_zsh_completion(entities, alias_to_canonical, actions_by_entity)
    (COMPLETION_DIR / "lucius.zsh").write_text(zsh_completion)
    print(f"Generated: {COMPLETION_DIR / 'lucius.zsh'}")

    fish_completion = generate_fish_completion(entities, alias_to_canonical, actions_by_entity)
    (COMPLETION_DIR / "lucius.fish").write_text(fish_completion)
    print(f"Generated: {COMPLETION_DIR / 'lucius.fish'}")

    ps_completion = generate_powershell_completion(entities, alias_to_canonical, actions_by_entity)
    ...
    print("  Bash:  source deployment/shell-completions/lucius.bash")
    print("  Zsh:   source deployment/shell-completions/lucius.zsh")
    print("  Fish:  source deployment/shell-completions/lucius.fish")
    print("  PowerShell: . deployment/shell-completions/lucius.ps1")
