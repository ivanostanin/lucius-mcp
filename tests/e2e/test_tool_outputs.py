...
async def test_create_tool_success_output(
    allure_client: AllureClient, cleanup_tracker: CleanupTracker, project_id: int, run_name: str
):
    """Verify create tool output is human friendly."""
    result = await create_test_case(project_id=project_id, name=run_name, description="Testing tool output")
