...
async def test_e2e_hierarchy_assign_test_cases_to_suite(
    ...
    run_id = uuid4().hex[:8]
    suite = await service.create_test_suite(name=f"E2E-Assign-Suite-{run_id}")
...
async def test_e2e_hierarchy_tools_smoke(
    ...
    create_output = await create_test_suite(name=f"E2E-Tool-Suite-{run_id}", project_id=project_id)
