...
async def test_custom_field_values_crud(allure_client: AllureClient) -> None:
    ...
    page = await service.list_custom_field_values(custom_field_name=field_name)
    ...
    updated_values = []
    for _ in range(5):
        page_after_update = await service.list_custom_field_values(custom_field_name=field_name)
        updated_values = page_after_update.content or []
        if any(v.id == created.id and v.name == updated_name for v in updated_values):
            break
        await asyncio.sleep(0.5)

    if not any(v.id == created.id and v.name == updated_name for v in updated_values):
        pytest.skip("Custom field value rename not reflected in list output.")

    assert any(v.id == created.id and v.name == updated_name for v in updated_values)
