...
- `create_launch`, `list_launches`, `get_launch`, `get_test_run_result`, `delete_launch`, `upload_test_results`, `list_launch_test_results`, `rerun_test_results_manually`, `start_manual_test_session`, `submit_manual_test_results`, `add_test_result_attachment`, `add_test_step_attachment`
...
- **Steps**:
  ...
  5. **Upload Results**: `upload_test_results(launch_id=LAUNCH_ID, results=[{"test_case_id":TC_UPLOAD_ID,"status":"passed","message":"External validation complete"}])`
     - Expectation: Output equals `Successfully uploaded 1 results to launch LAUNCH_ID`.
  ...
  6a. **Inspect Exact Result**: For a TestOps URL such as `/launch/LAUNCH_ID/tree/RESULT_ID?treeId=UI_STATE`, call `get_test_run_result(launch_id=LAUNCH_ID, test_result_id=RESULT_ID)`.
  ...
  8. **Submit Manual Results**: `submit_manual_test_results(test_session_id=SESSION_ID, results=[...])`
...
- **Prerequisite**: Complete Scenario 13 through its issue-linking steps and retain `LINK_TC_ID`.
- **Steps**:
  ...
  4. **Invalid Test Case ID**: `unlink_issue_from_test_case(test_case_id=0, issue_id="PROJ-7004")`
     - Expectation: A standardized validation error states that the test case ID must be a positive integer.
  5. **Cleanup**: `delete_test_case(test_case_id=LINK_TC_ID, confirm=true)` and `delete_defect(defect_id=LINK_DEFECT_ID, confirm=true)`
     - Expectation: Cleanup operations succeed.

#### 15. Cleanup Archived Entities
...
#### 17. Generate Test Code
- **Scenario source**: `specs/implementation-artifacts/7-7-test-code-generator.md`
- **Goal**: Generate an automation scaffold from a current TestOps test case.
- **Steps**:
